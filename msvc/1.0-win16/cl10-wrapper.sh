#!/bin/sh
# cl10 wrapper — stage the vendored MSVC 1.0 tree into a fresh DOSBox
# C: drive and run CL.EXE headless (uses the shared wrapper helpers).
#
# Invoke:  cl10 <source.c> [flags...]
#
# Design notes (same contract as the cl16/cl15 wrappers):
...
#   is copied back to /work as <source-stem>.OBJ.
...
sandbox=$(mktemp -d /tmp/cl10.XXXXXX)
trap 'rm -rf "$sandbox"' EXIT

cp -r /opt/msvc10/. "$sandbox"/
