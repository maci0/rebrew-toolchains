...
# Entrypoint — psyq/3.5-ps1
# cc wrapper — PSY-Q 3.5 pipeline (host cpp -> CC1PSX -> ASPSX -> obj parser).
#
...
rebrew_dosemu_run "$_work" /opt/psyq-3.5 "D:\\COMPILE.BAT"
...
rebrew_dosemu_run "$_work" /opt/psyq-3.5 "ASPSX.EXE -quiet D:\\output.s -o D:\\output.obj"
if [ ! -s "$_work/output.obj" ]; then
    _note=$(rebrew_dosemu_failure_note)
    rebrew_die "aspsx produced no object for $SRC$_note"
fi

/opt/psyq-3.5/psyq-obj-parser "$_work/output.obj" -o "$OUT_ABS"
