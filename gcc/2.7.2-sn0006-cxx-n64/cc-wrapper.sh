...
/usr/bin/cpp -E -lang-c++ -undef -D__GNUC__=2 -D__cplusplus -Dmips -D__mips__ -D__mips -Dn64 -D__n64__ -D__n64 -D_PSYQ -D__EXTENSIONS__ -D_MIPSEB -D__CHAR_UNSIGNED__ -D_LANGUAGE_C_PLUS_PLUS "$SRC_ABS" | ( rebrew_run /opt/gcc-2.7.2-sn0006-cxx-n64/cc1pln64.exe  $CC_FLAGS -o out.s ) \
    || rebrew_die "cc1pln64 failed on $SRC"
# shellcheck disable=SC2310  # same subshell contract as the cc1pln64 stage
( rebrew_run /opt/gcc-2.7.2-sn0006-cxx-n64/asn64.exe -q -G0 out.s -o out.obj ) \
    || rebrew_die "asn64 failed on the assembly cc1pln64 produced"
/opt/gcc-2.7.2-sn0006-cxx-n64/psyq-obj-parser out.obj -o "$OUT_ABS" -b -n -s
