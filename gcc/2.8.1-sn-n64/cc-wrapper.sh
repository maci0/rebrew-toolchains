...
/usr/bin/cpp -E -lang-c -undef -D__GNUC__=2 -Dmips -D__mips__ -D__mips -Dn64 -D__n64__ -D__n64 -D_PSYQ -D__EXTENSIONS__ -D_MIPSEB -D__CHAR_UNSIGNED__ "$SRC_ABS" | ( rebrew_run /opt/gcc-2.8.1-sn/cc1n64.exe  $CC_FLAGS -o out.s ) \
    || rebrew_die "cc1n64 failed on $SRC"
# shellcheck disable=SC2310  # same subshell contract as the cc1n64 stage
( rebrew_run /opt/gcc-2.8.1-sn/asn64.exe -q -G0 out.s -o out.obj ) \
    || rebrew_die "asn64 failed on the assembly cc1n64 produced"
/opt/gcc-2.8.1-sn/psyq-obj-parser out.obj -o "$OUT_ABS" -b -n
